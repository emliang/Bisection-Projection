    ...
    def opt_solve(self, X, solver_type='cvxpy', tol=1e-5):
        if solver_type == 'cvxpy':
            ...
            with mp.Pool(processes=n_process) as pool:
                ...
                Y = pool.map(solve_problem, params)
            sols = np.array(Y)
        else:
    ...
    def opt_solve(self, X, solver_type='cvxpy', tol=1e-5):
        if solver_type == 'cvxpy':
            ...
            with mp.Pool(processes=n_process) as pool:
                ...
                results = pool.map(solve_problem, params)
    ...
    def opt_solve(self, X, solver_type='cvxpy', tol=1e-5):
        if solver_type == 'cvxpy':
            ...
            with mp.Pool(processes=n_process) as pool:
                ...
                results = pool.map(solve_problem, args)
...
###################################################################
from pypower.api import makeYbus, rundcopf, makeBdc
...
class ACOPF_Problem:
    ...
    def __init__(self, data, test_size):
        ...
        self.output_L = torch.cat([self.pmin, self.qmin, self.vmin, -torch.ones_like(self.vmin)*torch.pi/2], dim=0).view(1,-1)
        self.output_U = torch.cat([self.pmax, self.qmax, self.vmax, torch.ones_like(self.vmax)*torch.pi/2], dim=0).view(1,-1)
    ...
    def load_mid_bus_info(self):
        ...
        self.newton_nomid_eqs_idx = np.concatenate([self.pflow_start_eqidx + self.pv,  # real power flow at non-slack gens
    ...
    def opt_solve(self, X, solver_type='pypower', tol=1e-5):
        ...
        if X.shape[0]>1:
            ...
            results = solve_problem(('acopf', 0, X_np[0], ppc, ppopt, \
                         idx_bus.PD, idx_bus.QD, idx_gen.PG, idx_gen.QG, idx_bus.VM, idx_bus.VA, \
    ...
    class PFFunctionFn(Function):
        ...
        def forward(ctx, X, Z):
            ...
            ctx.save_for_backward(-jac_vmva[:, last_eqs_idx, :], # jac_last_vars_vmva
                                  ...
                                  jac_vmva[:, newton_eqs_idx,:][:,:,partial_vm_yidx-2*data.ng]) # jac_newton_eq_partial_vm_var
    ...
    class PFFunctionFn(Function):
        ...
        def forward(ctx, X, Z):
            ...
            ctx.save_for_backward(-jac_vmva[:, last_eqs_idx, :],
                                  ...
                                  jac_mid_non_mid)
            ctx.data = [partial_vars_idx, partial_pg_yidx, partial_vm_yidx, newton_vars_idx, newton_mid_vars_idx, last_vars_idx, vm_start_yidx]
            return Y
