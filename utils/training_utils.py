    ...
    for n in range(total_iteration):
        ...
        if n%1000==0 and n>0:
            ...
            print(f'Iteration: {n}/{total_iteration}, '
                  ...
                  f'Test valid: {torch.mean(penalty_0).detach().cpu().numpy():.5f}',
                  end='\n')
...
def unsupervised_training_meip(model, constraints, n_dim, c_tensor, args):
    ...
    # Ctrain = constraints.trainX
    # Xtrain = constraints.trainY
    ...
    #     xt = model(Ctrain_batch)
    #     c_batch_extend = Ctrain_batch.view(-1, 1, c_dim).repeat(1, n_ip, 1)
    ...
    #     mse_loss = xt_scale.view(batch_size, n_ip, -1) - Xtrain_batch[:,constraints.partial_vars_idx].view(batch_size, 1, -1)
    ...
    for n in range(total_iteration):
        ...
        loss = args['w_penalty'] * infeasible_loss + args['w_ecc'] * feasible_loss
        loss.backward()
        optimizer.step()
        scheduler.step()
        iter_et = time.time()
        ...
        if (n + 1) % 1000 == 0:
            ...
            with torch.no_grad():
                ...
                penalty = torch.sum(torch.abs(violation), dim=-1)
                print(f'n_ip: {n_ip}, iteration: {n}, valid: {penalty.max():.4f}, ecc: {eccentricity_list[-1]:.4f}, sampling time: {sample_time:.4f}, training_time: {np.mean(training_time_list)}')
