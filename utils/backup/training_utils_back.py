from Homeomorphic_Projection.utils.nn_utils import *
from Homeomorphic_Projection.utils.sampling_utils import *
from Homeomorphic_Projection.utils.proj_utils import  *
import numpy as np
import torch
...
def train_nn_solver(data, args, save_dir):
    ...
    model = ResNet(in_dim, out_dim, int(data.intrin_dim*1.1), n_layer)
    model.to(DEVICE)
    solver_opt = optim.AdamW(model.parameters(),
                            lr=lr,
                            betas=(0.99, 0.999),
                            weight_decay=1e-6)
    ...
    if training_approach == 'supervise':
        for pn in range(pre_training):
            ...
            if 'Eq' in args['algoType']:
                ...
                mse_loss = loss(Z_pred_scale_batch, Ytrain_batch)
            mse_loss.backward()
            if pn % 100 == 0:
    ...
    for i in range(nepochs + 1):
        ...
        if training_approach == 'supervise':
            ...
            mse_loss = 0.5 * loss(Y_pred_scale_batch, Ytrain_batch) \
                       # +  loss1(Y_pred_scale_batch, Ytrain_batch)
            train_loss = mse_loss \
            ...
            train_loss = args['nn_para']['w_ineq'] * ineq_penalty.mean() \
                        + args['nn_para']['softWeightEqFrac'] * eq_penalty.mean() \
                        + 1000 * args['nn_para']['w_obj'] * training_obj.mean()
...
def test_nn_solver(data, args, model_save_dir, result_save_dir):
    ...
    model = torch.load(os.path.join(model_save_dir, f'{training_approach}_{NN_structure}.pth'), map_location=DEVICE)
    epoch_stats = {}
    with torch.no_grad():
    ...
    algo = args['algoType']
    training_approach =  args['nn_para']['approach']
    with open(os.path.join(result_save_dir, f'{training_approach}_{algo}_test_stats.dict'), 'wb') as f:
    ...
    if num_infeasible_prediction > 0:
        if args['proj_para']['useTestCorr']:
            if 'H_Proj' in args['algoType']:
                Yproj, steps = homeo_bisection(homeo_mapping, data, args, Z_pred[infeasible_index], X[infeasible_index])
            elif 'G_Proj' in args['algoType']:
                Yproj, steps = gauge_bisection(homeo_mapping, data, args, Z_pred[infeasible_index], X[infeasible_index])
            elif 'B_Proj' in args['algoType']:
                Yproj, _ = ip_bisection(feasible_ip, data, args, Z_pred[infeasible_index], X[infeasible_index])
...
def test_inf_time(data, args, model_save_dir, result_save_dir):
    ...
    algo = args['algoType']
    training_approach =  args['nn_para']['approach']
    epoch_stats = np.load(os.path.join(result_save_dir, f'{training_approach}_{algo}_test_stats.dict'), allow_pickle=True)
...
def unsupervised_training_mdh(model, constraints, x_tensor, c_tensor, args):
    ...
    for n in range(total_iteration):
        ...
        x_input = x_tensor[batch_index]
        batch_index = np.random.choice(range(c_tensor.shape[0]), batch_size*2, replace=True)
        c_input = c_tensor[batch_index]
        ...
        xt_scale = constraints.scale(c_input, xt)
        xt_full = constraints.complete_partial(c_input, xt_scale)
        eq_vio = constraints.eq_resid(c_input, xt_full).abs()
        ineq_vio = constraints.ineq_resid(c_input, xt_full).abs()
        # eq_penalty = (0.5 * eq_vio ** 2 + eq_vio).sum(-1)
        # ineq_penalty = (0.5 * ineq_vio ** 2 + ineq_vio).sum(-1)
        eq_penalty = eq_vio.sum(-1)
        ineq_penalty = ineq_vio.sum(-1)
        penalty = eq_penalty + ineq_penalty
        ...
        if n%1000==0 and n>0:
            model.eval()
            with torch.no_grad():
                batch_index = np.random.choice(range(c_tensor.shape[0]), batch_size, replace=True)
                c_input = c_tensor[batch_index]
...
def train_mdh_mapping(data, args, save_dir):
    ...
    # model = torch.load(os.path.join(save_dir, 'mdh_mapping.pth'), map_location=DEVICE)
...
def unsupervised_training_meip(model, constraints, n_dim, c_tensor, args):
    ...
    training_time_list = []
    model.train()
    Xtrain = constraints.trainX.to(DEVICE)
    Ytrain = constraints.trainY.squeeze().to(DEVICE)
    loss2 = nn.MSELoss()
    loss1 = nn.L1Loss()
    for n in range(1000):
        batch_index = np.random.choice(np.arange(Xtrain.shape[0]), 512)
        c_batch = Xtrain[batch_index]
        x_batch = Ytrain[batch_index]
        xt = model(c_batch)
        ...
        x_batch = x_batch.view(-1, 1, x_batch.shape[1]).repeat(1,n_ip,1).view(-1, x_batch.shape[1])
        ip_batch_extend = ip_batch.view(-1, n_dim)
        xt_scale = constraints.scale(c_batch_extend, ip_batch_extend)
        mse_loss = 0.5 * loss2(xt_scale, x_batch[:, constraints.partial_vars_idx]) \
                   # + loss1(xt_scale, x_batch[:, constraints.partial_vars_idx]) #+ penalty
        mse_loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        print(mse_loss.item(), end='\r')
    ...
    for n in range(total_iteration):
        ...
        c_batch_1 = c_tensor[batch_index]
        # xt = model(c_batch)
        # c_batch_extend = c_batch.view(-1, 1, c_dim).repeat(1, n_ip, 1)
        # c_batch_extend = c_batch_extend.view(-1, c_dim)
        # ip_batch = xt.view(-1, n_ip, n_dim)
        ...
        # violation = constraints.ineq_resid(c_batch_extend, xt_full)
        # penalty = ( 0.5*violation.abs()**2 + violation.abs()).sum(-1)
        # penalty = penalty.view(-1, n_ip).mean(dim=1)


        batch_index = np.random.choice(np.arange(Xtrain.shape[0]), batch_size)
        c_batch_2 = Xtrain[batch_index]
        c_batch = torch.cat([c_batch_1, c_batch_2],dim=0)
        x_batch = Ytrain[batch_index]
        xt = model(c_batch)
        c_batch_extend = c_batch.view(-1, 1, c_dim).repeat(1, n_ip, 1)
        c_batch_extend = c_batch_extend.view(-1, c_dim)
        ...
        eq_vio = constraints.eq_resid(c_batch_extend, xt_full).abs()
        ineq_vio = constraints.ineq_resid(c_batch_extend, xt_full).abs()
        # eq_penalty = (0.5 * eq_vio ** 2 + eq_vio).sum(-1)
        # ineq_penalty = (0.5 * ineq_vio ** 2 + ineq_vio).sum(-1)
        eq_penalty = eq_vio.sum(-1)
        ineq_penalty = ineq_vio.sum(-1)
        penalty = (eq_penalty + ineq_penalty).view(-1, n_ip).mean(dim=1)

        # ### infeasible loss function
        infeasible_index_1 = penalty[:batch_size] > 1e-5
        infeasible_index_2 = penalty[batch_size:] > 1e-5
        #
        mse_loss = 0.5 * loss2(xt_full[batch_size:][infeasible_index_2], x_batch[infeasible_index_2]) \
                   # +  loss1(xt_full[batch_size:][infeasible_index_2], x_batch[infeasible_index_2])

        infeasible_loss = (penalty).sum(-1).mean()
        ...
        loss =  mse_loss + \
                args['w_penalty'] * infeasible_loss + \
                n_ip*args['w_ecc'] * feasible_loss
        loss.backward()
        optimizer.step()
        scheduler.step()
        iter_et = time.time()
        ...
        if (n) % 1000 == 0:
            ...
            with torch.no_grad():
                ...
                penalty_max = torch.max(torch.abs(violation), dim=-1)[0]
                penalty_mean = torch.sum(torch.abs(violation), dim=-1)
                print(f'n_ip: {n_ip}, iteration: {n}, penalty: {penalty_mean.cpu().numpy().mean():.4f}, ',
                      f'feasibility: {(penalty_max <  1e-3).cpu().numpy().mean():.4f},',
                      f'ecc: {eccentricity_list[-1]:.4f}, sampling time: {sample_time:.4f}, training_time: {np.mean(training_time_list)}')
...
def unsupervised_training_mdg(model, constraints, x_tensor, xs_tensor, c_tensor, args):
    ...
    for n in range(total_iteration):
        ...
        loss = -  torch.mean(volume)   \
                +  w_penalty * torch.mean(penalty) \
                +  w_distortion * torch.mean(logdis) \
...
# def general_boundary_sampling(data, c_tensor, feasible_ip, num_boundary_point, bisect_step=10, sigma=0.01):
#     batch_size = feasible_ip.shape[0]
#     n_ip = feasible_ip.shape[1]
#     n_dim = feasible_ip.shape[-1]
#     c_dim = c_tensor.shape[-1]
#     # direction sampling
#     unit_vec = torch.randn(size=[batch_size * n_ip * num_boundary_point, n_dim]).to(feasible_ip.device) * sigma
#     # repeat for multiple IPs and extend to 2-dim vectors
#     feasible_ip = feasible_ip.view(batch_size, n_ip, 1, n_dim).repeat(1, 1, num_boundary_point, 1)
#     feasible_ip_extend = feasible_ip.view(-1, n_dim)
#     c_tensor = c_tensor.view(batch_size, n_ip, 1, c_dim).repeat(1, 1, num_boundary_point, 1)
...
# def objective_aware_sampling(data, c_tensor, feasible_ip, solution, num_boundary_point, bisect_step=10, sigma=0.1):
#     batch_size = feasible_ip.shape[0]
#     n_ip = feasible_ip.shape[1]
#     n_dim = feasible_ip.shape[-1]
#     c_dim = c_tensor.shape[-1]
#     # noise sampling
#     noise = torch.randn(size=[batch_size * n_ip * num_boundary_point, n_dim]).to(feasible_ip.device) * sigma
#     normalized_solution  = data.inverse_scale(None, solution)
#     normalized_solution = normalized_solution[:, data.partial_vars_idx]
#     normalized_solution = normalized_solution.view(batch_size,1,1,n_dim)
#     infeasible_pred = normalized_solution + noise.view(batch_size, n_ip, num_boundary_point, n_dim)
#     infeasible_pred_extend = infeasible_pred.view(-1, n_dim)
#     # repeat for multiple IPs and extend to 2-dim vectors
#     feasible_ip = feasible_ip.view(batch_size, n_ip, 1, n_dim).repeat(1, 1, num_boundary_point, 1)
#     feasible_ip_extend = feasible_ip.view(-1, n_dim)
#     c_tensor = c_tensor.view(batch_size, n_ip, 1, c_dim).repeat(1, 1, num_boundary_point, 1)
...
#     for _ in range(bisect_step):
#         am = (al + au)/2
#         xt = feasible_ip_extend + am * (infeasible_pred_extend - feasible_ip_extend)
#         xt_scale = data.scale(c_tensor_extend, xt)
#         xt_full = data.complete_partial(c_tensor_extend, xt_scale)
#         penalty = data.check_feasibility(c_tensor_extend, xt_full).sum(dim=1)
#         # penalty = penalty.view(num_feasible_points, num_interior_points, num_boundary_point, 1)
#         infeasible_index = penalty > 1e-5
#         feasible_index = penalty <= 1e-5
#         au[infeasible_index] = am[infeasible_index]
#         al[feasible_index] = am[feasible_index]
#     boundary_samples = feasible_ip_extend + al * (infeasible_pred_extend - feasible_ip_extend)
#     return boundary_samples.view(batch_size, n_ip, num_boundary_point, n_dim)
