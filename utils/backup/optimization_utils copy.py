...
class Base_Problem:
    ...
    def ineq_partial_grad(self, X, Y):
        ...
        for n in range(Y.shape[0]):
            ...
            # y[0, self.partial_vars_idx] = Y_pred
...
class QP_Problem(Base_Problem):
    ...
    def __init__(self, dataset, valid_frac=0.2, test_frac=0.0):
        ...
        # self.M = 2 * (self.G[:, self.partial_vars_idx] -
        #                 self.G[:, self.other_vars] @ (self.A_other_inv @ self.A_partial))
        # self.IM = 2 * (self.I[:, self.partial_vars_idx] -
        #                 self.I[:, self.other_vars] @ (self.A_other_inv @ self.A_partial))
    ...
    # def auto_ineq_partial_grad(self, X, Y):
    #     grad_list = []
    #     for n in range(Y.shape[0]):
    #         Y_pred = Y[n, self.partial_vars_idx].view(1, -1)
    #         x = X[n].view(1, -1)
    ...
    # def ineq_grad(self, X, Y):
    #     resids = torch.clamp(self.ineq_resid(X, Y), 0)
    #     l = torch.clamp(self.L - Y, 0)
    #     u = torch.clamp(Y - self.U, 0)
    #     return 2 * (resids @ self.G + l @ self.I + u @ self.I)

    # def ineq_partial_grad(self, X, Y):
    #     grad = torch.clamp(Y @ self.G.T - self.h, 0) @ self.M
    #     grad_U = torch.clamp(Y - self.U, 0) @ self.IM
    #     grad_L = torch.clamp(self.L - Y, 0) @ self.IM
    #     grad = grad + grad_L + grad_U
    #     Y = torch.zeros(X.shape[0], self.ydim, device=X.device)
    #     Y[:, self.partial_vars_idx] = grad
...
class QCQP_Probem(QP_Problem):
    ...
    #     #  h: 1 * m
    #     # """
    #     # yh = torch.matmul(self.H, Y.T).permute(2, 0, 1)
    #     # yhy = 0.5 * (yh * Y.view(Y.shape[0],1,-1))
    #     # grad = []
    #     # for i in range(self.nineq):
    #     #     grad = torch.clamp(yhy + Y @ self.G.T - self.h, 0) @ (self.G.T+yh) @ self._M
    #     # grad_U = torch.clamp(Y - self.U, 0) @ self._IM
    #     # grad_L = torch.clamp(self.L - Y, 0) @ self._IM
    #     # grad = grad + grad_L + grad_U
    #     # Y = torch.zeros(X.shape[0], self.ydim, device=X.device)
    #     # Y[:, self.partial_vars_idx] = grad
...
class SOCP_Probem(QP_Problem):
    ...
    def ineq_partial_grad(self, X, Y):
        ...
        for n in range(Y.shape[0]):
            ...
            ineq_penalty = self.ineq_dist(x, y) ** 2
            ineq_penalty = torch.sum(ineq_penalty, dim=-1, keepdim=True)
...
class SDP_Probem(Base_Problem):
    ...
    def ineq_partial_grad(self, X, Y):
        ...
        for n in range(Y.shape[0]):
            ...
            ineq_penalty = self.ineq_dist(x, y)
            ineq_penalty = torch.sum(ineq_penalty, dim=-1, keepdim=True)
    ...
    def ineq_resid(self, X, Y):
        ...
        num_sample = 1024
        est = torch.rand(size=(Y.shape[0], self.ymdim, num_sample), device=X.device) * 2 - 1 # batch * n * k
        ...
        ## direct methodsd
        # pel = -1*torch.det(Ym/100).view(-1,1)

        # pel,_ = torch.slogdet(Ym)
        # pel = torch.linalg.eigvals(Ym)
...
class ACOPF_Problem:
    ...
    def __init__(self, data, valid_frac=0.2, test_frac=0.0):
        ...
        Ybus, _, _ = makeYbus(self.baseMVA, ppc_copy['bus'], ppc_copy['branch'])
        Ybus = Ybus.todense()
        ...
        self.pg_init = ppc['gen'][:, idx_gen.PG] / self.baseMVA
        self.qg_init = ppc['gen'][:, idx_gen.QG] / self.baseMVA
    ...
    def eq_jac(self, Y):
        ...
        Ir = vr @ Yr - vi @ Yi
        Ii = vr @ Yi + vi @ Yr
        # print(cosva.shape, Ir.shape, Yi.shape, Ydiagv(Yr, -vi).shape)
        # print(1/0)
        ...
        dreal_dvm = -mdiag(cosva, Ir) - dtm(vr, Ydiagv(Yr, cosva) - Ydiagv(Yi, sinva)) \
                    - mdiag(sinva, Ii) - dtm(vi, Ydiagv(Yi, cosva) + Ydiagv(Yr, sinva))
        dreal_dva = -mdiag(-vi, Ir) - dtm(vr, Ydiagv(Yr, -vi) - Ydiagv(Yi, vr)) \
                    - mdiag(vr, Ii) - dtm(vi, Ydiagv(Yi, -vi) + Ydiagv(Yr, vr))
        ...
        dreact_dvm = mdiag(cosva, Ii) + dtm(vr, Ydiagv(Yi, cosva) + Ydiagv(Yr, sinva)) \
                     - mdiag(sinva, Ir) - dtm(vi, Ydiagv(Yr, cosva) - Ydiagv(Yi, sinva))
        dreact_dva = mdiag(-vi, Ii) + dtm(vr, Ydiagv(Yi, -vi) + Ydiagv(Yr, vr)) \
                     - mdiag(vr, Ir) - dtm(vi, Ydiagv(Yr, -vi) - Ydiagv(Yi, vr))
    ...
    def cal_penalty(self, X, Y):
        penalty = torch.cat([self.ineq_dist(X, Y), self.eq_resid(X, Y)], dim=1)
        return penalty
    ...
    def opt_solve(self, X, solver_type='pypower', tol=1e-5):
        ...
        for i in range(X_np.shape[0]):
            ...
            total_time += (end_time - start_time)
            print(i, end='\r')
            pg = my_result['gen'][:, idx_gen.PG] / self.baseMVA
            qg = my_result['gen'][:, idx_gen.QG] / self.baseMVA
    ...
    class PFFunctionFn(Function):
        ...
        def forward(ctx, X, Z):
            ...
            ctx.save_for_backward(torch.cat(jacs),
                                  torch.tensor(newton_vars_idx, device=X.device),
                                  torch.tensor(newton_eqs_idx, device=X.device))
            return Y
        ...
        def backward(ctx, dl_dy):
            ...
            data = ctx.data
            # jac, newton_jac_inv, newton_vars_idx, newton_eqs_idx = ctx.saved_tensors
            jac, newton_vars_idx, newton_eqs_idx = ctx.saved_tensors

            ## Step 2 (calc pg at slack and qg at gens)
            jac_pre_inv = jac[:, newton_eqs_idx, :]
            jac_pre_inv = jac_pre_inv[:, :, newton_vars_idx]

            # gradient of all voltages through step 3 outputs
            last_eqs_idx = np.concatenate([data.pflow_start_eqidx + data.slack, data.qflow_start_eqidx + data.spv])
            last_vars_idx = np.concatenate([
                data.pg_start_yidx + data.slack_, np.arange(data.qg_start_yidx, data.qg_start_yidx + data.ng)])
            jac3 = jac[:, last_eqs_idx, :]
            dl_dvmva_3 = -jac3[:, :, data.vm_start_yidx:].transpose(1, 2).bmm(
                dl_dy[:, last_vars_idx].unsqueeze(-1)).squeeze(-1)
            ...
            # d_int = newton_jac_inv.transpose(1, 2).bmm(dl_dy_total[:, newton_vars_idx].unsqueeze(-1)).squeeze(-1)
            d_int = torch.linalg.solve(jac_pre_inv.transpose(1, 2),
                                       dl_dy_total[:, newton_vars_idx].unsqueeze(-1)).squeeze(-1)
...
def PFFunction_forward(data, tol=1e-5, bsz=1024, max_iters=10):
